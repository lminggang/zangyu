...
class  StockinController extends \Admin\Controller\AdminController {
    ...
    public function index()
    {
        ...
        $this->assign('invoice_type', C('STOCK.INVOICE_TYPE'));
    ...
    public function add_stockin()
    {
        ...
        if($_POST){
            ...
            $ret = $StockinModel
                ->addStockinInfo(
                    $stockin_date, $staff_id, $transactor_id, $invoice_type,
            ...
            $this->assign('invoice_type', C('STOCK.INVOICE_TYPE'));
            $this->assign('depreciation_time', json_encode(C('STOCK.DEPRECIATION_TIME')));
            $this->assign('staff_list', $staffList);
    ...
    public function formatProductItem()
    {
        ...
        foreach($_POST['product_id'] as $key => $val){
            ...
            $invoice_id = intval($_POST['invoice_id'][$key]);

            $category_id = intval($_POST['category_id'][$key]);
            $buy_date = trim($_POST['buy_date'][$key]);
            $depreciation_time = intval($_POST['depreciation_time'][$key]);
            ...
            if($category_id > 0){
                if(empty($buy_date) || false === strtotime($buy_date)){
                    $res = array(
                        "errNo" => "1001",
                        "errMsg" => "请选择购入时间"
                    );
                ...
                if($depreciation_time < 1 or $depreciation_time > 5){
                    $res = array(
                        "errNo" => "1001",
                        "errMsg" => "折旧年限只能为 1-5 年"
                    );
                    $this->ajaxReturn($res);
                    exit;
                }

                $depreciation_time = $depreciation_time * 12;
            }else{
                $depreciation_time = 0;
                $buy_date = '';
            }
    ...
    public function edit_stockin()
    {
        ...
        if($_POST){
            ...
            $ret = $StockinModel
                ->updateStockinInfo(
                    $stockin_id, $staff_id, $transactor_id, $invoice_type,
                    $QC_Bill_Number, $purchase_order_num, $contract_num, $product_item
                );
