...
def get_webloc_file_list():
	webloc_file_list = glob.glob('people_all/*.webloc')
...
def run():
	...
	for webloc_file in webloc_file_list:
		...
		if glob.glob('people_news/{}*'.format(task_id)):
			continue
		if num % 1000 == 0:
			time.sleep(1)
		...
		# p = subprocess.Popen(['python ./input_url.py "{}" people_news'.format(page_addr)], shell=True)
...
def save_html(task_id, content):
	save_path = 'people_news/{}.html'.format(task_id)
	save(save_path, content)

def save_webloc(task_id, content):
	save_path = 'people_news/{}.webloc'.format(task_id)
	save(save_path, content)
