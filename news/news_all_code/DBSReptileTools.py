...
class DBSReptileTools:
    ...
    def get_html(self, page_addr='', timeout=0):
        '''
            获取html
            params:
        ...
        try:
            response = requests.get(self.page_addr, timeout)
            ...
            if len(html) < 1000:
                # print(html)
                raise Exception('html 小于 1w 字符，请检查是否需要输入验证码。 [{}]'.format(page_addr))
    ...
    def get_webloc_page_addr(self, read_file_path= '', webloc_xml=''):
        ...
        if not page_addr:
            raise Exception('webloc file not url')
        return page_addr[0]
    ...
    def get_task_id(self, page_addr=''):
        '''
            获取task_id
            params:
                page_addr str [页面地址]
